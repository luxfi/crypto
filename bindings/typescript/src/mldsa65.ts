...
function sizes(): { pk: number; sk: number; sig: number } {
  if (_pkSize === null) {
    const ffi = getFFI();
    _pkSize = ffi.lux_mldsa65_pk_size();
    _skSize = ffi.lux_mldsa65_sk_size();
    _sigSize = ffi.lux_mldsa65_sig_size();
  }
...
export function keypair(): [Uint8Array, Uint8Array] {
  ...
  const rc = ffi.lux_mldsa65_keypair(pkBuf, pkLen, skBuf, skLen);
  if (rc !== 0) throw new Error(`lux_mldsa65_keypair failed: ${rc}`);
...
export function sign(secretKey: Uint8Array, message: Uint8Array): Uint8Array {
  ...
  const rc = ffi.lux_mldsa65_sign(skBuf, skBuf.length, msgBuf, msgBuf.length, sigBuf, sigLen);
  if (rc !== 0) throw new Error(`lux_mldsa65_sign failed: ${rc}`);
...
export function verify(publicKey: Uint8Array, message: Uint8Array, signature: Uint8Array): boolean {
  ...
  const rc = ffi.lux_mldsa65_verify(pkBuf, pkBuf.length, msgBuf, msgBuf.length, sigBuf, sigBuf.length);
  return rc === 0;
}
