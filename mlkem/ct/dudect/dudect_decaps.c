...
int main(int argc, char **argv) {
    ...
    int rc = mlkem_decaps_ct_setup();
    if (rc != 0) {
        fprintf(stderr, "mlkem_decaps_ct_setup failed: rc=%d\n", rc);
        return 1;
    }
    ...
    if (state == DUDECT_LEAKAGE_FOUND) {
        fprintf(stderr, "VERDICT: leakage detected in mlkem.Decaps\n");
        return 2;
    }
